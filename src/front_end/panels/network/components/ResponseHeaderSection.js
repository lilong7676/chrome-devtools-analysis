...
export class ResponseHeaderSection extends HTMLElement {
    ...
    #markOverrides() {
        ...
        this.#headerEditors.filter(header => header.name === 'set-cookie').forEach(header => {
            if (this.#request?.originalResponseHeaders.find(originalHeader => originalHeader.name === 'set-cookie' &&
    ...
    #render() {
      ...
      ${headerDescriptors.map((header, index) => html `
        <${HeaderSectionRow.litTagName} .data=${{
            header: header,
        }} @headeredited=${this.#onHeaderEdited} @headerremoved=${this.#onHeaderRemoved} @enableheaderediting=${this.#onEnableHeaderEditingClick} data-index=${index}></${HeaderSectionRow.litTagName}>
      `)}
