...
export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
     ...
     */
    setBreakpointItems(breakpointItems) {
    ...
    breakpointLocations(event) {
        if (event.target instanceof Element) {
            return this.breakpointLocationsForElement(event.target);
        }
        return [];
    }
    breakpointLocationsForElement(element) {
    ...
    breakpointCheckboxClicked(event) {
        const hadFocus = this.hasFocus();
        ...
        for (const breakpoint of breakpoints) {
            ...
            const item = this.breakpoints.find(breakpointItem => breakpointItem.locations.some(loc => loc.breakpoint === breakpoint));
            if (item) {
    ...
    revealLocation(element) {
        const uiLocations = this.breakpointLocationsForElement(element).map(breakpointLocation => breakpointLocation.uiLocation);
        let uiLocation = null;
    ...
    breakpointContextMenu(event) {
        ...
        contextMenu.defaultSection().appendItem(removeEntryTitle, () => breakpoints.map(breakpoint => void breakpoint.remove(false /* keepInStorage */)));
        if (event.target instanceof Element) {
            contextMenu.defaultSection().appendItem(i18nString(UIStrings.revealLocation), this.revealLocation.bind(this, event.target));
        }
        ...
        if (breakpoints.some(breakpoint => !breakpoint.enabled())) {
            ...
            if (event.target instanceof Element) {
                ...
                contextMenu.defaultSection().appendItem(enableInFileTitle, this.toggleAllBreakpointsInFile.bind(this, event.target, true));
            }
        }
        if (breakpoints.some(breakpoint => breakpoint.enabled())) {
            ...
            if (event.target instanceof Element) {
                ...
                contextMenu.defaultSection().appendItem(disableInFileTitle, this.toggleAllBreakpointsInFile.bind(this, event.target, false));
            }
        }
