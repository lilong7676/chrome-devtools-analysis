...
import { buildStackTraceAsCallFramesFromId, data as samplesData, getAllHotFunctionsBetweenTimestamps, } from './SamplesHandler.js';
...
export function treify(events, options) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.name === "ScheduleStyleRecalculation" /* KnownEventName.ScheduleStyleRecalculation */) {
            lastScheduleStyleRecalcEvent = event;
        }
        if (event.name === "InvalidateLayout" /* KnownEventName.InvalidateLayout */) {
            lastInvalidateLayout = event;
        }
        if (event.name === "RecalculateStyles" /* KnownEventName.RecalculateStyles */) {
...
function eventIsLongTask(event, mustBeOnMainFrame) {
    if (event.name !== "RunTask" /* KnownEventName.RunTask */) {
        return false;
    }
