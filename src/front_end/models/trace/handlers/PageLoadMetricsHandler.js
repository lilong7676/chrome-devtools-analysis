...
function estimateTotalBlockingTimes() {
    ...
    for (const [frameId, metricsByNavigation] of metricScoresByFrameId) {
        for (const [navigationId, metrics] of metricsByNavigation) {
            const navigationTBT = metrics.get("TBT" /* MetricName.TBT */);
            const navigationFCP = metrics.get("FCP" /* MetricName.FCP */);
            ...
            for (const rootId of mainThreadTree.roots) {
                ...
                if (task.name !== "RunTask" /* KnownEventName.RunTask */ || Types.TraceEvents.isTraceEventInstant(task)) {
                    continue;
                }
